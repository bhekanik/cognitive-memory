...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ```typescript
    async searchLexical(queryText: string, filters?: MemoryFilters): Promise<ScoredMemory[]> {
...
2. Include cold memories only when `include_cold=True` / `includeCold=true`
3. Include superseded memories only when `include_superseded=True` / `includeSuperseded=true`
4. Exclude stubs unless `include_stubs=True` / `includeStubs=true`
5. Return results sorted by similarity score, descending
6. Truncate to `top_k`
...
<Tabs syncKey="lang">
  <TabItem label="Python">
    ...
    async def search_similar(
        ...
        results = []
        candidates = await self.all_active() if include_cold else await self.all_hot()
  ...
  <TabItem label="TypeScript">
    ...
    async vectorSearch(
      ...
      const candidates = filters.includeCold
        ? await this.allActive()
        : await this.allHot();

      return candidates
        .filter((memory) => {
          if (filters.userId && memory.userId !== filters.userId) return false;
          if (memory.embedding.length === 0) return false;
          if (memory.isStub && !filters.includeStubs) return false;
          if (memory.isSuperseded && !filters.includeSuperseded) return false;
          return true;
        })
        .map((memory) => ({
          ...memory,
          relevanceScore: cosineSimilarity(queryEmbedding, memory.embedding),
          finalScore: cosineSimilarity(queryEmbedding, memory.embedding),
        }))
...
- Separate key prefixes or collections
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ```typescript
    async transaction<T>(callback: (adapter: MemoryAdapter) => Promise<T>): Promise<T> {
      return callback(this); // no-op for non-transactional backends
    }
    ```
  </TabItem>
</Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    describe("MyAdapter", () => {
      it("stores and retrieves memories", async () => {
        ...
        const id = await adapter.createMemory(memory);

        await expect(adapter.getMemory(id)).resolves.toMatchObject({
...
- [Adapter specification](/cognitive-memory/api/adapter-spec/) — canonical contract documentation
- [InMemory adapter source](https://github.com/planetaryescape/cognitive-memory/blob/main/sdks/python/src/cognitive_memory/adapters/memory.py) — reference implementation
