...
<GroupedBarChart client:visible
  data={[
    { metric: 'Overall', cogMem: 44.8, rag: 20 },
    { metric: 'Multi-hop', cogMem: 48.5, rag: 15 },
  ]}
  xKey="metric"
  bars={[
...
Cognitive Memory's association graph links related memories. Retrieving one activates its neighbors. The multi-hop gap is where this mechanism matters most.
...
But for multi-session agents that need to reason across conversations, track evolving facts, and handle contradictions, a lifecycle memory system gives you control that naive RAG does not try to provide.
