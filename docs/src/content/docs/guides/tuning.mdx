...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    // Search with: limit=20, deepRecall=false
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      extractionMode: "raw",
      retrievalScoreExponent: 0.1, // don't penalize old turns
      runMaintenanceDuringIngestion: false, // no consolidation
    };
    // Search with: limit=50, deepRecall=false
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      ...
      retrievalScoreExponent: 0.2,
      directBoost: 0.1,
    };
    // Search with: limit=30, deepRecall=true
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      extractionMode: "semantic",
      retrievalScoreExponent: 0.1, // nearly ignore decay
      coreAccessThreshold: 5, // promote to core faster
      coreStabilityThreshold: 0.7,
      coldStorageTtlDays: 365, // keep cold memories longer
      consolidationRetentionThreshold: 0.1, // only consolidate very faded
      customExtractionInstructions:
        "Focus on medical conditions, medications, allergies, and treatment history.",
    };
    // Search with: limit=60, deepRecall=true
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      extractionMode: "semantic",
      retrievalScoreExponent: 0.7, // strong recency bias
      directBoost: 0.05, // less stability reinforcement
      coldMigrationDays: 3, // move to cold faster
      coldStorageTtlDays: 30, // purge cold data quickly
      consolidationRetentionThreshold: 0.3, // consolidate earlier
    };
    // Search with: limit=10, deepRecall=false
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  <TabItem label="Python">
    ...
    # Run maintenance once at the end — resolves queued conflicts, cold migration, TTL expiry, consolidation
    await mem.tick()
    ```
  </TabItem>
  <TabItem label="TypeScript">
    ...
    declare const conversations: string[];
    declare const llm: LLMProvider;
    ...
    for (const [index, conv] of conversations.entries()) {
      await mem.extractAndStore(conv, `archive-${index + 1}`, llm);
    }
...
Disabling maintenance during ingestion avoids cold-migration checks and conflict resolution LLM calls after every batch. Conflict candidates are still tagged during ingestion (zero-cost cosine checks) and resolved in bulk when you call `tick()` at the end.
...
`extraction_mode` controls how conversations become memories. See [Ingestion Pipeline](/cognitive-memory/concepts/ingestion/) for detailed comparison.
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    // Medical assistant
    const medicalConfig: Partial<CognitiveMemoryConfig> = {
      customExtractionInstructions:
        "Focus on medical conditions, medications, dosages, allergies, and treatment outcomes. Classify all medical facts as core.",
    };

    // E-commerce
    const ecommerceConfig: Partial<CognitiveMemoryConfig> = {
      customExtractionInstructions:
        "Track product preferences, sizes, brands, past purchases, and return history.",
    };
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  <TabItem label="Python">
    ```python
    response = await mem.search(query, deep_recall=True)
    results = response.results
    ```
  </TabItem>
  <TabItem label="TypeScript">
    ```typescript
    const response = await mem.search({ query, deepRecall: true });
