...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ```typescript
    await mem.store({
      content: "User attended a Python conference in Portland on March 5",
      category: "episodic",
      importance: 0.5,
    });
    ```
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ```typescript
    await mem.store({
      content: "User does 30 minutes of yoga every morning",
      category: "procedural",
      importance: 0.6,
    });
    ```
...
See [Core promotion](/cognitive-memory/concepts/core-promotion/) for details on the promotion mechanism.
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ```typescript
    await mem.store({
      content: "User's name is Alex, 32 years old",
      category: "core",
      importance: 0.95,
    });
    ```
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const category: MemoryCategory = "core";
    ```
  </TabItem>
</Tabs>
...
Expired `plan` and `transient_state` memories are filtered from search results by default. See [Deep Recall](/cognitive-memory/concepts/deep-recall/) for how to include them.
