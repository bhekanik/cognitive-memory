...
## Our results (latest full run — 10 conversations)

Configuration: latest SDK scoring and lifecycle settings, evaluated on LoCoMo categories 1-4.
...
<HeatmapTable
  ...
  rows={[
    ...
    { label: 'Average', values: [44.8, 35.4, 48.5, 31.5] },
  ]}
/>
...
Going from narrow to broader recall improved overall retrieval quality. The extra candidates contain relevant memories that rank poorly by embedding similarity alone but are critical for answering the question.
