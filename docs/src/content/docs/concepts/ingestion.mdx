  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      customExtractionInstructions:
        "Focus on the user's dietary restrictions and food preferences.",
    };
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const embeddings = await this.embeddingProvider.embedBatch?.(contents)
      ?? await Promise.all(contents.map((content) => this.embeddingProvider.embed(content)));
...
No LLM calls happen at this stage. A single `search_similar(top_k=5)` call is made per memory, and its results are used for both conflict tagging (pairs above 0.85) and stability reinforcement in Stage 4 (pairs above 0.75). This keeps ingestion fast regardless of how many memories exist.
...
See [Conflict detection](/cognitive-memory/concepts/conflict-detection/) for the full two-phase resolution mechanism.
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    for (const existing of similar) {
      if (existing.relevanceScore > 0.75 && existing.id !== memory.id) {
        await this.adapter.updateMemory(existing.id, {
          stability: Math.min(1.0, existing.stability + 0.05),
        });
      }
    }
    ```
...
See [Associations](/cognitive-memory/concepts/associations/) for details on how these links are used during retrieval.
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    // High-level: extract + store everything from a conversation
    const memoryIds = await mem.extractAndStore(
      conversationText,
      "session-42",
      llm,
    );
    ...
    // Low-level: add a single memory directly (skips LLM extraction)
    const memoryId = await mem.store({
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      extractionMode: "semantic", // "raw" | "semantic" | "hybrid"
      extractionModel: "gpt-4o-mini", // LLM for extraction
      embeddingModel: "text-embedding-3-small", // embedding model
      embeddingDimensions: 1536, // embedding vector size
      runMaintenanceDuringIngestion: true, // set false for batch imports
      customExtractionInstructions: null, // prepend to extraction prompt
    };
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    for (const [index, conversation] of conversations.entries()) {
      await mem.extractAndStore(conversation, `archive-${index + 1}`, llm);
    }
