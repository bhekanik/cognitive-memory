...
### `get_batch(memory_ids) -> list[Memory]` / `getMemories(ids) -> Memory[]`
...
### `search_similar(query_embedding, top_k, include_superseded, include_cold, include_stubs, user_id) -> list[tuple[Memory, float]]`
...
| `include_cold` | `bool` | False | Include cold memories (for deep recall) |
| `include_stubs` | `bool` | False | Include archived stubs. Normal engine calls keep this false. |
| `user_id` | `str \| None` | None | Restrict results to one user when the adapter stores multiple users |

Returns: List of `(Memory, similarity_score)` sorted by similarity descending.

Must exclude:
- Cold memories (unless `include_cold=True`)
- Stub memories (unless `include_stubs=True`; the engine keeps this false for normal and deep recall)
- Superseded memories (unless `include_superseded=True`)
- Memories without embeddings

Normal `search()` calls use `include_cold=False`, `include_superseded=False`, and `include_stubs=False`. Deep recall uses `include_cold=True`, `include_superseded=True`, and `include_stubs=False`.

### `search_lexical(query_text, top_k, include_superseded, include_cold, include_stubs, user_id) -> list[tuple[Memory, float]]` *(optional)*
...
| `top_k` | `int` | 10 | Max results |
| `include_superseded` | `bool` | False | Include superseded memories |
| `include_cold` | `bool` | False | Include cold memories |
| `include_stubs` | `bool` | False | Include archived stubs |
| `user_id` | `str \| None` | None | Restrict results to one user |
...
**TypeScript:** `searchLexical(queryText: string, filters?: MemoryFilters): Promise<ScoredMemory[]>` — also optional, defaults to returning an empty array.
  ...
  <TabItem label="TypeScript">
    ...
    async transaction<T>(callback: (adapter: MemoryAdapter) => Promise<T>): Promise<T> {
      return this.pool.transaction(async (client) => {
        return callback(new TransactionAdapter(client));
      });
    }
