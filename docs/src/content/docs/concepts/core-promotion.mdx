...
| `session_ids` | >= 3 distinct | Whether the memory is relevant across contexts |
...
Core memories can be demoted when contradicted. If a new memory conflicts with a core memory (detected by the [conflict detection](/concepts/conflict-detection/) system), the core memory is demoted to semantic:
