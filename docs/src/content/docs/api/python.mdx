...
Canonical retrieval API. Search memories with retention-weighted scoring, associative expansion, optional deep recall, optional reranking, and retrieval boosting.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `deep_recall` | `bool` | `False` | Include cold and superseded records in the candidate set. Stubs stay excluded. |
...
| `combined_score` | `float` | Semantic relevance weighted by retention^alpha |
| `is_associative` | `bool` | Came via association link |
| `via_deep_recall` | `bool` | From a cold or superseded candidate included by deep recall |
