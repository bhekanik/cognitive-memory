...
| `rerank_factor` | `int` | `1` | Multiplier for the retrieval candidate pool before reranking; final results are sliced back to requested `top_k` |
...
| `rerankFactor` | `number` | `1` | Multiplier for the retrieval candidate pool before reranking; final results are sliced back to requested `limit` |
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    const config: Partial<CognitiveMemoryConfig> = {
      ...
      associationDecayConstantDays: 90.0,

      // Consolidation
      consolidationRetentionThreshold: 0.2,
...
See the [Tuning guide](/cognitive-memory/guides/tuning/) for detailed advice on adjusting these parameters for your workload.
